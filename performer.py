...
def perform():
    # logger.info(f'Запуск от {datetime.datetime.now().date()}')
    ...
    for idx, row in enumerate(ws.iter_rows(min_row=0)):
        ...
        # folder_dates = row[4].value  # written by ; if many
    ...
    while True:
        ...
        row = c.fetchone()
        if row:
            # if 'Перевод собственных средств на свой счет в другом Банке.' in row[11]:
            #     print(row[0], row[11])
            #     c.close()
            #     conn.close()
            #     continue
            ...
            break
    # logger.info('Создание платежных поручений завершено')

    #    folder_date переменная для файла парковки.
    # logger.info('Запуск загрузки парковки')
    upload_parking_process()
    # logger.info('Загрузка парковки завершена')
    ...
    if res:
        ...
        if not received_file:
            logger.info("Попробовали загрузить в 1с 2 раза. Но не получилось. Закончили работу")
            return
            ...
            ...
        # Step 7: Общая сумма операций из xls файла на сетке должна быть равна сумме Сумма покупки минус
        # Сумма возврата в 1С загрузке

        # logger.info(f'Сверка Sales Report')
    ...
    # logger.info(f'Загрузка и сверка завершена', 'Завершение')
