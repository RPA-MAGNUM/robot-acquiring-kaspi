...
from config import odines_username_rpa, odines_password_rpa, sed_username, sed_password, bool_use_prod_1c

from rpamini import App, clipboard_get, clipboard_set, kill_process_list, ApplicationException, RobotException, \
    BusinessException, Web
...
class Odines(App):
    ...
    def run(self) -> None:
        ...
        self.find_element({
            ...
            "visible_only": True, "enabled_only": True, "found_index": 0
        }).click(double=True) if bool_use_prod_1c else self.find_element({
            "title": "Информационная база", "class_name": "", "control_type": "ListItem",
            "visible_only": True, "enabled_only": True, "found_index": 0
        }).click(double=True)
        sleep(3)
        ...
        self.find_element({
            ...
            "found_index": 0
        }).type_keys(odines_username_rpa if bool_use_prod_1c else "beerobot", self.keys.TAB, click=True, clear=True,
                     protect_first=True)
        self.find_element({
            "title": "", "class_name": "", "control_type": "Edit", "visible_only": True, "enabled_only": True,
            "found_index": 0
        }).set_text(odines_password_rpa if bool_use_prod_1c else "Br123456")
        self.find_element({
        ...
        conf_message = self.wait_element(
            {"title": "Конфигурация базы данных не соответствует сохраненной конфигурации.\nПродолжить?",
             "class_name": "", "control_type": "Pane", "visible_only": True, "enabled_only": True, "found_index": 0},
            timeout=3)
        if conf_message:
            self.find_element(
        ...
        self.root = self.find_element(self.root_selector, timeout=1)
        self.parent_back(1)
